                ...
                if self.byte[i] == 0xE5 or self.byte[i] == 0x00:
                    ...
                    if self.byte[i + 11] == 0x10:  # Directory
                        ...
                        if self.byte[j - 32] == 0x0F:
                            ...
                            for t in range(0, subEntry, 1):
                                ...
                                while check:
                                    for k in range(10, 0, -2):
                                        if (
                                            (
                                                self.byte[j - k - index] == 0x00
                                                and self.byte[j - k + 2 - index] == 0xFF
                                            )
                                            or (
                                                self.byte[j - k - index] == 0x00
                                                and self.byte[j - k + 2 - index] == 0x00
                                            )
                                            or (
                                                self.byte[j - k - index] == 0xFF
                                                and self.byte[j - k + 2 - index] == 0xFF
                                            )
                                        ):
                                            check = False
                                            break
                                        temp += (
                                            self.byte[j - k - index : j - k + 2 - index]
                                        ).decode("utf-8")
                            ...
                            self.DIR_Name = (
                                (self.byte[i : i + 8]).decode("utf-8").rstrip()
                            )
                        # created time
                        ...
                        self.DIR_CrtDate = self.DIR_CrtDate.strftime(
                            "%m/%d/%Y"
                        )  # converted to mdy
                        ################
                        # next cluster
                        ...
                        if self.DIR_NxtClus != 0:
                            tempData.update(
                                {
                                    "contents": self.detectDIR(
                                        offset
                                        + (self.DIR_NxtClus - root)
                                        * self.BPB_BytesPerSec
                                        * self.BPB_SecPerClus,
                                        64,
                                        self.DIR_NxtClus,
                                    )
                        ...
                        if self.byte[j - 32] == 0x0F:
                            ...
                            self.DIR_Name = (
                                (self.byte[i : i + 8]).decode("utf-8").rstrip()
                                + "."
                                + (self.byte[i + 8 : i + 11]).decode("utf-8")
                            )
                        self.DIR_Attr = "File"
                        # created time
                        temp = self.byte[i + 13 : i + 16]
        ...
        if Cluster == 0x2:
            for i in range(0, len(self.detectDIR(offset, 128, self.BPB_RootClus)), 1):
                self.info_dict.update(
                    {i: self.detectDIR(offset, 128, self.BPB_RootClus)[i]}
                )
...
RDET = (
    ...
    + DRIVE.getBootSector()["BPB_NumFATs"]
    * DRIVE.getBootSector()[  # calculate the Root Directory Entry Table
        "BPB_SecsPerFAT"
    ]
    + DRIVE.getBootSector()[  # RDET = Reserved Sector Count + Number of FATs * Sectors per FAT + Starting Offset
        "StartingOffset"
    ]  # RDET in FAT32 is stored in the root directory
) * DRIVE.getBootSector()["BPB_BytesPerSec"]

DRIVE.FindDirectory(
    RDET, DRIVE.getBootSector()["BPB_RootClus"]
)  # find the directory of FAT32 drive
